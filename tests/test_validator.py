...
class TestProjectEscapeHatchIgnored:
    """A project-level config cannot downgrade a BLOCKED rule.

    Restricted to user-level config for the same reason as `whitelist` — a cloned
    repo auto-loads project config on `cd`.
    """
...
"""
    ...
    @staticmethod
    def _write_config(tmp_path, monkeypatch, project=None, user=None):
        """Lay out a temp HOME and cwd, writing whichever config bodies were given."""
        if project is not None:
            hooks = tmp_path / ".claude" / "hooks"
            hooks.mkdir(parents=True)
            (hooks / "schlock-config.yaml").write_text(project)
        if user is not None:
            user_dir = tmp_path / ".config" / "schlock"
            user_dir.mkdir(parents=True)
            (user_dir / "config.yaml").write_text(user)
        home = tmp_path if user is not None else tmp_path / "nonexistent"
    ...
    def test_escape_hatch_stripped_from_project_config(self, tmp_path, monkeypatch, caplog):
        ...
        self._write_config(tmp_path, monkeypatch, project=self.PROJECT_HATCH)
    ...
    def test_project_config_without_hatch_logs_no_warning(self, tmp_path, monkeypatch, caplog):
        ...
        self._write_config(tmp_path, monkeypatch, project="\nrule_overrides:\n  recursive_delete:\n    enabled: false\n")
    ...
    def test_user_level_hatch_still_downgrades(self, tmp_path, monkeypatch):
        ...
        clear_caches()
        self._write_config(tmp_path, monkeypatch, user=self.USER_HATCH)
    ...
    @pytest.mark.parametrize(
        ...
        [
            USER_HATCH,
            # A bare hatch with no risk_level of its own: the user consented to no
            # downgrade at all, yet the property-level merge would still let the
            # project file supply one.
            "\nrule_overrides:\n  disk_destruction_dd:\n    allow_blocked_override: true\n",
        ],
        ids=["user-chose-a-level", "user-left-a-bare-hatch"],
    )
    def test_project_risk_level_cannot_ride_a_user_hatch(self, tmp_path, monkeypatch, user_config, caplog):
        """A project file naming no hatch still cannot relevel a rule the user hatched."""
        clear_caches()
        self._write_config(tmp_path, monkeypatch, project=self.PROJECT_RELEVEL, user=user_config)

        result = validate_command("dd if=/dev/zero of=/dev/sda")

        # The project's SAFE is refused; the user's own level, or the floor, stands.
        assert result.risk_level != RiskLevel.SAFE
        assert "disk_destruction_dd" in result.matched_rules
        assert "Ignoring risk_level for ['disk_destruction_dd'] in project config" in caplog.text

    def test_project_config_may_still_close_a_user_hatch(self, tmp_path, monkeypatch):
        """`allow_blocked_override: false` in project config is a refusal, not an escalation.

        Stripping the key regardless of its value would delete that refusal and turn a
        deny into an allow, so the strip mirrors rules.py's `is True` check exactly.
        """
        clear_caches()
        self._write_config(
            tmp_path,
