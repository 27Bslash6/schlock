...
def _paramexpand(parserobj, string, sindex):
    ...
    if c and c in "0123456789$#?-!*@":
        ...
        node = ast.node(kind="parameter", value=string[sindex + 2 : zindex], pos=(sindex, zindex + 1))
        # TODO
        ...
        if temp1:
            return (ast.node(kind="parameter", value=temp1[1:], pos=(sindex, zindex)), zindex)
    ...
    while True:
        ...
        if c in "<>":
            if (
                ...
                parts.append(ast.node(kind="processsubstitution", command=node, pos=(tindex - 2, sindex[0])))
        ...
        #     pass
            ...
            if (
                ...
                stopatcolon = wordtoken.flags & set([flags.word.ASSIGNRHS, flags.word.ASSIGNMENT, flags.word.TILDEEXP])
                expand = True
                ...
                if i > sindex[0] and expand:
                    node = ast.node(kind="tilde", value=string[sindex[0] : i], pos=(sindex[0], i))
                    parts.append(node)
            ...
            if nextchar() == "`":
                ...
                if wordtoken.flags & flags.word.NOCOMSUB:
                    ...
                    node = ast.node(kind="commandsubstitution", command=command, pos=(tindex, sindex[0]))
                    parts.append(node)
                    istring += string[tindex : sindex[0]]
    ...
    if parts:
        ...
        class v(ast.nodevisitor):
            def visitnode(self, node):
                ...
                node.pos = (node.pos[0] + wordtoken.lexpos, node.pos[1] + wordtoken.lexpos)
