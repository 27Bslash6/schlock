...
class tokenizer:
    def __init__(
        self, s, parserstate, strictmode=True, eoftoken=None, lastreadtoken=None, tokenbeforethat=None, twotokensago=None
    ):
    ...
    def _readtokenword(self, c):
                ...
                if c == "\\":
                    ...
                    if peek_char == "\n":
                        ...
                        if (
                            cd is None
                            or cd == "`"
                            or (cd == '"' and peek_char is not None and "dquote" in sh_syntaxtab[peek_char])
                        ):
        ...
        if (
            d["all_digit_token"]
            and (c in "<>" or self._last_read_token.ttype in (tokentype.LESS_AND, tokentype.GREATER_AND))
        ...
        if not d["dollar_present"] and not d["quoted"] and self._reserved_word_acceptable(self._last_read_token):
            if tokenword in valid_reserved_first_command:
                ...
                if (
                    ps & parserflags.CASEPAT
                    and ttype != tokentype.ESAC
                    or ttype == tokentype.TIME
                    and not self._time_command_acceptable()
                ):
                    pass
        ...
        while count:
            ...
            if reservedwordok:
                if c.islower():
                    ...
                    lexrwlen += 1
                    continue
                if lexrwlen == 4 and _shellbreak(c):
                    if ret[-4:] == "case":
                        insidecase = True
                    elif ret[-4:] == "esac":
                        insidecase = False
        ...
        while count:
            ...
            if insidecomment:
                ...
                if c == "\n":
                    insidecomment = False
                continue
            if lookforcomments and not insidecomment and c == "#" and (not ret or ret[-1] == "\n" or _shellblank(ret[-1])):
                insidecomment = True
            ...
            if c == close:
                count -= 1
            elif open != close and sawdollar and open == "{" and c == open or not firstclose and c == open:
                count += 1
    ...
    def _command_token_position(self, token):
        return (
            ...
            or (
                token.ttype not in (tokentype.SEMI_SEMI, tokentype.SEMI_AND, tokentype.SEMI_SEMI_AND)
    ...
    def _specialcasetokens(self, tokstr):
        if (
            ...
            and self._token_before_that.ttype in (tokentype.FOR, tokentype.CASE, tokentype.SELECT)
            and tokstr == "in"
        ):
