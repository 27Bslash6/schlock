...
def p_shell_command(p):
    ...
    if len(p) == 2:
        ...
        p[0] = ast.node(
            kind="compound", redirects=[], list=[ast.node(kind=kind, parts=parts, pos=_partsspan(parts))], pos=_partsspan(parts)
        )
...
def p_for_command(p):
    ...
    p[0] = ast.node(
        kind="compound", redirects=[], list=[ast.node(kind="for", parts=parts, pos=_partsspan(parts))], pos=_partsspan(parts)
    )
...
def p_if_command(p):
    ...
    parts = _makeparts(p)
    p[0] = ast.node(
        kind="compound", redirects=[], list=[ast.node(kind="if", parts=parts, pos=_partsspan(parts))], pos=_partsspan(parts)
    )
