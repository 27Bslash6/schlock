...
| `git` | `status`, `log`, `diff` | `-c alias.X=!cmd`, `-c core.sshCommand`, `-c core.pager`, `-c credential.helper`, `-c diff.external`, `-c merge.tool`, `-c man.*`, `-c help.format`; writing `man.*` / `help.format` with `git config`, including a `--rename-section` into or out of them (rated on the key, whatever the value, since they pick the program `git help` runs: HIGH at the top level, BLOCKED inside `$()`, and a worse value keeps its own verdict); writing or injecting `filter.*`, `mergetool.*`, `difftool.*`, `core.gitProxy`, `core.alternateRefsCommand`, `uploadpack.packObjectsHook` (value judged); writing path-valued `core.hooksPath`, `core.askpass`, `core.fsmonitor`, `gpg.program`, `credential.helper` (rated on the key: HIGH at the top level, BLOCKED inside `$()`; an empty value and a `core.fsmonitor` boolean stay unrated); writing `include.path`, `includeIf.*.path`, `init.templateDir` (BLOCKED on the key at both tiers: they load config or hooks from a file; the top-level BLOCKED applies to a direct `git` call only, so behind a wrapper or `find -exec` the write rates HIGH); a `--rename-section` into or out of any section above (rated on the name, since a rename never shows the value it moves: HIGH at the top level, BLOCKED inside `$()`, and BLOCKED at both tiers for a bootstrap section) |
